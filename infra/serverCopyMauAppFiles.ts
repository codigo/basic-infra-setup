...
export function copyMauAppDataFilesToServer(server: pulumi.Output<Server>, publicIp: pulumi.Output<string>) {
  ...
  const createMauAppFolders = new command.remote.Command("create tooling data folders", {
    connection,
  ...
  const restoreMauAppData = new command.remote.Command("restore pocketbase data", {
    connection,
    create: pulumi.interpolate`
